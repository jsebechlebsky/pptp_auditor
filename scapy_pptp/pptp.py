...
from scapy.fields import XByteField, ByteEnumField, XIntField,\
        IntField, IntEnumField, XShortField, ShortField, ShortEnumField,\
        StrFixedLenField, LenField, FlagsField
...
class PPTPStartControlConnectionRequest(PPTP):
    ...
    fields_desc = [LenField("len", 156),
                   ...
                   StrFixedLenField("host_name", None, 64),
                   StrFixedLenField("vendor_string", None, 64)]
...
class PPTPStartControlConnectionReply(PPTP):
    ...
    fields_desc = [LenField("len", 156),
                   ...
                   FlagsField("bearer_capabilities", 0, 32,
                              _PPTP_BEARER_CAPABILITIES_FLAGS),
                   ShortField("maximum_channels", None),
                   ShortField("firmware_revision", None),
                   StrFixedLenField("host_name", None, 64),
                   StrFixedLenField("vendor_string", None, 64)]
...
class PPTPStopControlConnectionReply(PPTP):
    ...
    fields_desc = [LenField("len", 16),
                   ...
                   XShortField("reserved_0", 0x0000),
                   ByteEnumField("result", 1,
                                 _PPTP_stop_control_connection_result),
                   ByteEnumField("error", 0, _PPTP_general_error_code),
                   XShortField("reserved_2", 0x0000)]
...
class PPTPOutgoingCallRequest(PPTP):
    ...
    fields_desc = [LenField("len", 168),
                   ...
                   ShortField("pckt_window_size", 10),
                   ShortField("pckt_proc_delay", 0),
...
class PPTPOutgoingCallReply(PPTP):
    ...
    fields_desc = [LenField("len", 32),
                   ...
                   ShortField("peer_call_id", None),
                   ByteEnumField("result_code", 0, _PPTP_result_code),
                   ByteEnumField("error_code", 0, _PPTP_general_error_code),
                   ShortField("cause_code", None),
                   IntField("Connect speed", None),
                   ShortField("pckt_window_size", None),
                   ShortField("pckt_proc_delay", None),
                   IntField("channel_id", None)]
...
class PPTPIncomingCallRequest(PPTP):
    ...
    fields_desc = [LenField("len", 220),
                   ...
                   XShortField("reserved_0", 0x0000),
                   ShortField("call_id", None),
                   ShortField("call_serial_number", None),
                   IntEnumField("call_bearer_type", 1, _PPTP_bearer_type),
                   IntField("channel_id", None),
                   ShortField("dialed_number_len", None),
                   ShortField("dialing_number_len", None),
...
class PPTPIncomingCallReply(PPTP):
    ...
    fields_desc = [LenField("len", 24),
                   ...
                   ByteEnumField("error_code", 0, _PPTP_general_error_code),
                   ShortField("pckt_window_size", 64),
                   ShortField("pckt_transmit_delay", 0),
                   XShortField("reserved_1", 0x0000)]
...
class PPTPIncomingCallConnected(PPTP):
    ...
    fields_desc = [LenField("len", 28),
                   ShortEnumField("type", 11, _PPTP_msg_type),
                   XIntField("magic_cookie", _PPTP_MAGIC_COOKIE),
...
class PPTPWANErrorNotify(Packet):
    ...
    fields_desc = [LenField("len", 40),
                   ...
                   ShortField("peer_call_id", None),
                   XShortField("reserved_1", 0x0000),
                   IntField("CRC Errors", 0),
                   IntField("Framing Errors", 0),
                   IntField("Hardware Overruns", 0),
                   IntField("Buffer Overruns", 0),
                   IntField("Time-out Errors", 0),
                   IntField("Alignment Errors", 0)]


class PPTPSetLinkInfo(Packet):
